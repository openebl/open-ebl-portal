...
export const amendRequestEmailNotifier: EmailNotifier = async ({ db, service, platform, rec, newStash }) => {
  ...
  getLogger().info(`Send amend requested email to platform ${platform.id} users`);

  const notificationName = "amend_requested";
  const event = lastEvent(rec);
  const number = eBlNo(rec);
  const buList = await businessInfoList();
  const sender = buList?.[event?.amendment_request?.request_by ?? ""]?.legalBusinessName ?? "";
