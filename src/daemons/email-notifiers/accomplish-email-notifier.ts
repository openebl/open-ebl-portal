...
export const accomplishEmailNotifier: EmailNotifier = async ({ db, service, platform, rec, newStash }) => {
  ...
  const sender = buList?.[event?.accomplish?.accomplish_by ?? ""]?.legalBusinessName ?? "";
