...
import EditPlatformSection from "@/app/_components/admin/platforms/edit-platform-section";
...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const platform = await api.adminPlatform.get.query(BigInt(params.id));
  ...
  return (
    <PermissionContext session={session} permission="write:admin/platforms">
      <div className="px-12 py-10 font-content">
        ...
        <EditPlatformSection platform={platform} permissions={session?.permissions} />
      </div>
    </PermissionContext>
  );
}
