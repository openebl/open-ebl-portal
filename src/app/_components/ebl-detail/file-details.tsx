...
const FileDetailsLine = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="flex items-center justify-start">
    <div className="mr-8 w-[7rem] whitespace-nowrap font-normal text-light">{title}</div>
    {children}
  </div>
);
    ...
    <section className="flex flex-col items-start rounded-lg border border-solid border-[#DFE4E9] bg-white p-[1.875rem] shadow-lg">
      ...
      <div className="mt-[1.875rem] flex w-full items-stretch justify-between gap-5 self-stretch">
        ...
        <div className="flex flex-col justify-end">
          <Button className="flex h-[2.75rem] w-[11.25rem] items-center justify-center rounded-lg border border-secondary1 bg-white p-0 text-sm font-semibold">
            <a
              className="flex h-full w-full items-center justify-center"
              href={`/api/file/download/${id}/${fileName}`}
              target="_blank"
            >
              Download eBL
            </a>
