...
const EBlActionSchemaWithID = z.object({
  id: z.string(),
...
const performEBlAction = async (payload: {
  request: EBlActionSchemaWithIDType;
  action: string;
...
export const eBlRouter = createTRPCRouter({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const res = await fetch(
        `${env.BU_SERVER_URL}/ebl?offset=${input.offset}&limit=${input.limit}&status=${input.filter}&report=true`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const list = (await res.json()) as EBlRecordListType;

      // TODO: remove this after demo
      if (env.PANAMA_DEMO) {
        const result = await ctx.db.query.PaymentRequests.findMany({
          where: eq(PaymentRequests.payerBusinessUnitId, String(ctx.session.businessUnitId)),
        });

        const paymentNeeded = await Promise.all(
          result
            .filter((p) => p.status === "REQUESTED")
            .map(async (paymentRequest) => {
              const res = await fetch(`${env.BU_SERVER_URL}/ebl/${paymentRequest.eBlId}`, {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
                  Authorization: `Bearer ${env.BU_SERVER_API_KEY}`,
        ...
        return { ...list, records: [...paymentNeeded, ...(list.records ?? [])] };
      }
  ...
  updateDraft: protectedProcedure.input(EBlFormUpdateSchema).mutation(async ({ ctx, input }) => {
  ...
  amend: protectedProcedure.input(EBlFormAmendSchema).mutation(async ({ ctx, input }) => {
    const { ebl_id: id, ...rest } = input;
  ...
  transfer: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const request = {
        ...input,
        metadata: { username: ctx.session.user.name ?? "" },
        authentication_id: ctx.session.authenticationId,
      };
      return await performEBlAction({
        request,
        action: "transfer",
        business_unit_id: String(ctx.session.businessUnitId),
      });
    }),
