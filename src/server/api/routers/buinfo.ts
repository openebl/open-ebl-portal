...
export const buinfoRouter = createTRPCRouter({
  ...
  list: protectedProcedure.input(z.object({ keyword: z.string().optional() })).query(async ({ input }) => {
    const buList = await businessInfoList();
    if (!buList) return [];

    if (!input.keyword) return Object.entries(buList).map(([id, bu]) => ({ id, ...bu }));

    const filteredPlatforms = Object.entries(buList)
      .filter(([_id, bu]) => bu.legalBusinessName?.toLowerCase().includes(input.keyword!.toLowerCase()))
      .map(([id, bu]) => ({ id, ...bu }));
